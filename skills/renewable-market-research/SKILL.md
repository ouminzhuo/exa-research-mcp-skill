...
For first-batch data-contract integrity, every material project must split status into `developmentStage` and `activityStatus`, with `ledgerTreatment`, `projectCapacityTreatment`, `oemCapacityTreatment`, and `capacityScope` controlling admission and MW arithmetic. A paused, withdrawn, cancelled, or superseded project may remain visible in the project universe, but it must flow to Excluded inactive MW, not active confirmed/opportunity/unallocated MW.

OEM relationships must split into `oemRelationshipType` and `oemRelationshipStatus`. OEM contract expiry, termination, or supersession does not make an active project inactive; it changes the OEM bucket, usually to `oemCapacityTreatment=unallocated_mw`, while the project remains governed by `activityStatus` and `projectCapacityTreatment`. Do not use ambiguous "locked MW" or "锁定MW". OEM capacity views may use only: Firm MW, Committed MW, Influenced MW, Unallocated MW, and Excluded inactive MW.
...
6. **Verify, reconcile, and aggregate data**
   ...
   - Build `{slug}-project_ledger.json` or `{slug}-pipeline-ledger.json` from the validated main JSON and source/evidence records, not directly as a replacement for the rich master JSON. The ledger owns canonical project IDs, local/English/Chinese aliases, dedupe keys, source traces, evidence layers, `developmentStage`, `activityStatus`, `ledgerTreatment`, `projectCapacityTreatment`, `oemCapacityTreatment`, `capacityScope`, merge/reject decisions, `Opportunity MW`, OEM relationship fields, and fields in `schema/project-ledger.schema.json`.
   - Build `{slug}-metric_ledger.json`, `{slug}-policy_target_ledger.json`, `{slug}-auction_ledger.json`, `{slug}-oem_allocation_ledger.json`, and `{slug}-capacity_reconciliation.md/json` after source/evidence verification and before fact freeze. Use `schema/metric-ledger.schema.json`, `schema/policy-target-ledger.schema.json`, `schema/auction-ledger.schema.json`, `schema/oem-allocation-ledger.schema.json`, and `schema/capacity-reconciliation.schema.json` for the core data contracts. Core numeric fields must be `number` or `null`; unknown values require `valueStatus`/`displayValue` or field-specific status/display fields, not `"unknown"` inside numeric fields.
   - Build `{slug}-canonical_facts.json` as the only editable frozen fact source after the rich master JSON and all core ledgers are current. It must carry `factProfile`, `requiredFactTypes`, `conditionalFactTypes`, and `notApplicableFactTypes`, so markets without auctions, paused projects, public OEM relations, or draft targets do not need fake zero facts. Then generate `{slug}-fact_freeze.json` as a compatibility projection with `generatedFrom`, `canonicalFreezeId`, and `canonicalFactsHash`; do not edit it as a second fact source.
...
7. **Generate reports**
   ...
   - Every chapter must have `chapter_inputs/{slug}-chapter-*-input-manifest.json` before drafting. A chapter writer may read only the allowed frozen fact IDs, metric IDs, project IDs, policy target IDs, auction IDs, OEM allocation IDs, required disclosures, and source artifacts listed in that manifest. Source Markdown must mark key claims with machine-readable markers such as `{{fact:FACT-ID}}`, `{{metric:METRIC-ID|value=123|unit=MW}}`, `{{project:PROJECT-ID}}`, `{{policy:POLICY-TARGET-ID}}`, `{{auction:AUCTION-ID}}`, and `{{oem:OEM-ALLOCATION-ID}}`; IDs not allowed by the manifest block release.
...
8. **Validate and hand off**
   ...
   - Run `scripts/validate_market_integrity.py` or its PowerShell wrapper to identify duplicate candidates, missing source fields, missing source-to-final metadata, policy target records without legal backtrace, phase-order gaps, artifact ownership gaps, canonical fact/profile gaps, fact-freeze projection gaps, chapter input-manifest gaps, external chapter ID gaps, metric consistency gaps, scope disclosure gaps, capacity arithmetic gaps, OEM share/recompute gaps, numeric field contract gaps, project status conflicts, parent/phase rollup gaps, unit arithmetic gaps, release cleanliness gaps, deprecated values in chapter drafts, and release-gate/audit-content gaps.
